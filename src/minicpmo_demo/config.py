...
配置来源优先级（高 → 低）：
    1. CLI 参数（minicpmo_demo.server.worker / minicpmo_demo.server.gateway 的 argparse）
    2. 仓库根目录 config.json（gitignored）
    3. Pydantic 默认值（本文件中定义）

首次部署时，复制 configs/config.example.json 为 config.json 并修改 model_path：
...
from minicpmo_demo.paths import CONFIG_EXAMPLE_PATH, CONFIG_PATH

logger = logging.getLogger(__name__)

_CONFIG_PATH = str(CONFIG_PATH)
_EXAMPLE_PATH = str(CONFIG_EXAMPLE_PATH)
