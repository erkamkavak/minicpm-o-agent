...
from minicpmo_demo.gateway.models import (
    GatewayWorkerStatus,
    ServiceStatus,
    WorkersResponse,
    QueueStatus,
...
from minicpmo_demo.gateway.ref_audio_registry import (
    RefAudioRegistry,
    RefAudioListResponse,
    UploadRefAudioRequest,
    RefAudioResponse,
)
from minicpmo_demo.paths import PROJECT_ROOT
...
async def lifespan(app: FastAPI):
    ...
    # 初始化参考音频注册表
    data_dir = os.path.join(PROJECT_ROOT, "data", "assets", "ref_audio")
...
async def _session_cleanup_loop() -> None:
    ...
    from minicpmo_demo.server.session_cleanup import cleanup_sessions
...
def main():
    ...
    proto_group.add_argument("--http", action="store_true", help="降级为 HTTP")
    ...
    if use_https:
        ...
        logger.warning("Running in HTTP mode (no TLS).")

    # Bump WS max payload from uvicorn's 16 MiB default to 128 MiB so that
    # base64-encoded video attachments can be proxied to a worker without
    # being rejected with code 1009.
    uvicorn.run(
        app,
        host=args.host,
        port=port,
