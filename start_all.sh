...
# Pre-compile with: PYTHONPATH=src .venv/base/bin/python -m minicpmo_demo.tools.precompile
...
# ============ 配置 ============
# 从 minicpmo_demo.config 读取端口配置
PROJECT_DIR="$(cd "$(dirname "$0")" && pwd)"
VENV_PYTHON="$PROJECT_DIR/.venv/base/bin/python"
PYTHONPATH_DIR="$PROJECT_DIR/src"

GATEWAY_PORT=$($VENV_PYTHON -c "import sys; sys.path.insert(0,'$PYTHONPATH_DIR'); from minicpmo_demo.config import get_config; print(get_config().gateway_port)" 2>/dev/null || echo "10024")
WORKER_BASE_PORT=$($VENV_PYTHON -c "import sys; sys.path.insert(0,'$PYTHONPATH_DIR'); from minicpmo_demo.config import get_config; print(get_config().worker_base_port)" 2>/dev/null || echo "22400")
...
for GPU_ID in $(echo "$GPU_LIST" | tr ',' ' '); do
    ...
    nohup env CUDA_VISIBLE_DEVICES=$GPU_ID PYTHONPATH="$PYTHONPATH_DIR" $VENV_PYTHON -m minicpmo_demo.server.worker \
        --port $WORKER_PORT \
        --gpu-id $GPU_ID \
...
nohup env PYTHONPATH="$PYTHONPATH_DIR" $VENV_PYTHON -m minicpmo_demo.server.gateway \
    --port $GATEWAY_PORT \
